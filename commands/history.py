...
async def handle_history_command(msg_type, user_info, context_type, context_id, send_msg, count=None):
    try:
...
async def handle_clear_history_command(msg_type, user_info, context_type, context_id, send_msg, count=None):
    try:
        if count is not None:
            count = int(count)
            if count <= 0:
