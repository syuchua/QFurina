...
async def handle_command(command, msg_type, recipient_id, send_msg, context_type, context_id):
    ...
    try:
        if main_command == 'help':
            ...
            await handle_model_command(msg_type, recipient_id, args, send_msg)
        elif main_command == 'r18':
            if not args or args not in ['0', '1', '2']:
                raise ValueError("无效的R18模式参数")
            await handle_r18_command(msg_type, recipient_id, args, send_msg)
        ...
        await send_msg(msg_type, recipient_id, error_message)
